...
pid_t pid;
u16 icmp_req = 1;
struct timeval start, stop, Time; // set time use sec   ¡A time.h 
...
int main(int argc, char* argv[])
{
	...
	myicmp packet;  //struct to choose ip or icmp
	int timeout = DEFAULT_TIMEOUT;

	strcpy(device_name, argv[2]);
	strcpy(req.ifr_name, device_name);
	timeout = atoi(argv[4]); //string to int, input is sec
    ...
    //get ip address of my interface by SIOCGIFADDR read input's interface 
    if(ioctl(sockfd_send, SIOCGIFADDR, &req) < 0){
        ...
        myip = destination.sin_addr; // binary store
    }
	...
	char IP[INET_ADDRSTRLEN]; // u16 inet.h
   ...
   //______________________________________________________________
	...
	unsigned char splited_ip[ADDRLEN]; //store subnet ip each value (int)
  ...
  //_________________________________________________________________
   ...
   //find the start and end IP address of the subnet
	...
	if(splited_mask[2] == 255){
		...
		end_ip = start_ip + available_ip -2; //all ones is  for broadcast 
	...
	if(argc == 5){
		if(!strcmp(argv[0],"./ipscanner") && !strcmp(argv[1],"-i") && !strcmp(argv[3],"-t")){
			for(int i=start_ip; i<=end_ip; i++){
				...
				Time.tv_sec = timeout / 1000; // time is msec
				...
				while(1){
					...
					if(ntohs(packet.icmp_all.icmp_type) == ICMP_ECHOREPLY){
						printf("ICMP reply from: %s , time : %ld.%04ld ms",full_IP,timeSec,timeUsec);
						printf("\n");
						break;
					}	
				}
